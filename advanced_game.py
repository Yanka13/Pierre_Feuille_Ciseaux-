...
class Shifumi():
    ...
    def play(self):
        ...
        while self.score != self.max_score:
            ...
            self.user_result = input(
                "Veuillez choisir entre pierre, feuille, ciseaux")
            ...
            if self.user_result in self.HANDS:
                ...
                print(
                    f"Voici le résultat de la machine : {self.machine_result}")

                self.compare_hands()

                self.score = max(self.machine_score, self.user_score)
                print(
                    f"Votre score est : {self.user_score}, celui de la machine est {self.machine_score}")

            else:
                print(
                    f"Attention, veuillez saisir une valeur valide parmi cette liste : {self.HANDS}")
                continue
